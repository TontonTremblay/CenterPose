...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
        ...
        if opt.pre_hm_hp:
            self.pre_hm_hp_layer = nn.Sequential(
    ...
    def forward(self, x, pre_img=None, pre_hm=None, pre_hm_hp=None):
        y = []
        x = self.base_layer(x)
        ...
        if pre_hm_hp is not None:
            x = x + self.pre_hm_hp_layer(pre_hm_hp)
        for i in range(6):
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
                 last_level, head_conv, out_channel=0, use_convGRU=False, opt=None):
        super(DLASeg, self).__init__()
        assert down_ratio in [2, 4, 8, 16]
        self.opt = opt
        self.first_level = int(np.log2(down_ratio))
        self.last_level = last_level
        self.base = globals()[base_name](pretrained=pretrained, opt=opt)
    ...
    def forward(self, x, pre_img=None, pre_hm=None, pre_hm_hp=None):
        ...
        if (pre_hm is not None) or (pre_img is not None) or (pre_hm_hp is not None):
            x = self.base(x, pre_img, pre_hm, pre_hm_hp)
        else:
            x = self.base(x)
